...
static jlong nativeInit(JNIEnv *env, jobject thiz) {
    if (!Charset.self) {
        ...
        MainActivity.self = FindClassOrDie(env,  "com/linux/x11/MainActivity");
...
LorieViewResources::LorieViewResources(JNIEnv *callerEnv, jobject view) {
    ...
    jfieldID activityField = env->GetFieldID(env->GetObjectClass(view), "activity", "Lcom/linux/x11/MainActivity;");
...
JNIEXPORT jint JNI_OnLoad(JavaVM *vm, __unused void *reserved) {
    ...
    if (!strcmp(__progname, "com.linux.x11")) {
    ...
    vm->AttachCurrentThread(&env, nullptr);
    jclass cls = env->FindClass("com/linux/x11/LorieView");
