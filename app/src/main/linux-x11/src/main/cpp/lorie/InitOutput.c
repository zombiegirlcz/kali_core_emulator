...
void OsVendorInit(void) {
    ...
    lorieServerDebugEnabled = getenv("LINUX_X11_DEBUG") != NULL;
...
void ddxReady(void) {
    ...
    if (NoListenAll)
        return;
    if (!xstartupArgv) {
        if (xstartup && !strlen(xstartup)) // allow overriding $LINUX_X11_XSTARTUP with empty xstartup arg
            return;
        if (!xstartup || !strlen(xstartup))
            xstartup = getenv("LINUX_X11_XSTARTUP");
        if (!xstartup || !strlen(xstartup))
            return;
    }
...
Bool loriePresentFlip(__unused RRCrtcPtr crtc, __unused uint64_t event_id, __unused uint64_t target_msc, PixmapPtr pixmap, __unused Bool sync_flip) {
    ...
    char *forceFlip = getenv("LINUX_X11_FORCE_FLIP");
