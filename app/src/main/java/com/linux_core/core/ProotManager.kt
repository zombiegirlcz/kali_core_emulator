...
object ProotManager {
    ...
    fun setupProotEnvironment(
        ...
        // daemon's environment, so it never sees the variables built below.
        // Persisting them lets boot reconstruct the session settings for the
        // sudo path (see the NH_ENV_FROM_APP guard in the boot script).
        try {
            File(nhDir, "root_env").writeText(
                buildString {
                    append("# generated by ProotManager from root_settings - do not edit\n")
                    append("NH_ISOLATED='").append(nhIsolated).append("'\n")
            ...
            Log.w(TAG, "Failed to write root_env: ${e.message}")
        }

        val envVars =
            mutableListOf(
                "NH_MOUNT_STORAGE=$mountStorageFlag",
