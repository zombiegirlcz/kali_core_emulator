...
object RootfsManager {
    ...
    fun distroRootfsDir(
        context: Context,
        distroId: String,
    ): File = File(context.filesDir, "$NH_DISTRO_DIR/$distroId")

    fun dockerRootfsDir(
        context: Context,
        imageName: String,
    ): File = File(context.filesDir, "$NH_DISTRO_DIR/docker/$imageName")

    fun backupDir(context: Context): File = File(context.filesDir, "$NH_DISTRO_DIR/backup")
        ...
        flow {
            ...
            val url =
                try {
                    ...
                    val allowedHosts =
                        listOf("images.kali.org", "kali.download", "raw.githubusercontent.com", "deb.parrot.sh", "archive.parrotsec.org")
                    ...
                    XZCompressorInputStream(tracked).use { xzIn ->
                        TarArchiveInputStream(xzIn).use { tarIn ->
                            ...
                            while (entry != null) {
                                ...
                                val relativeName =
                                    if (stripPrefix != null && entry.name.startsWith(stripPrefix!!)) {
            ...
            try {
                ...
                class CountingInputStream(input: java.io.InputStream) : FilterInputStream(input) {
                    ...
                    override fun read(
                        ...
                        off: Int,
                        len: Int,
                    ): Int = super.read(b, off, len).also { if (it > 0) bytesRead += it }
                }
    ...
    suspend fun pullRootfsFromUrl(
        ...
        flow {
            ...
            val fileBase =
                parsedUrl.path.substringAfterLast('/').ifEmpty { "rootfs" }
    ...
    suspend fun importLocalRootfsFile(
        ...
        flow {
            ...
            if (validExts.none { nameLower.endsWith(it) }) {
                throw IOException(
                    "Unsupported archive format for '${archiveFile.name}'. Supported formats: .tar.gz, .tgz, .tar.xz, .txz, .tar, .tar.bz2",
                )
            }
    ...
    private fun copyFilesRecursive(src: File, dst: File) {
        if (src.isDirectory) {
            dst.mkdirs()
            src.listFiles()?.forEach { copyFilesRecursive(it, File(dst, it.name)) }
        } else {
