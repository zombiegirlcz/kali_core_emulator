...
- **`boot`** (Android Host): Universal PRoot launcher deployed to `filesDir/usr/bin/boot` (and guest `/usr/local/bin/boot`). It sets environment variables (`HOME=/root`, `USER=root`, `PATH` from guest rootfs, `TERM=xterm-256color`, `LANG=C.UTF-8`), performs diagnostics, checks for dynamic loader combinations (`proot` + `loader` + `libtalloc.so.2`), and launches the guest shell with appropriate flag mounts (`-v 0 --kill-on-exit --link2symlink -0 --sysvipc`). Supports `kali`, `parrot`, and `docker:<image>` modes, plus re-entry mode (`boot -- <cmd>`) for root bridge.
